...
document.addEventListener("drop", function(event) {
  ...
  app.controller('myCtrl123', function($scope, $filter, $http,$location,shareDataService) {
	 ...
	 for (i = 0; i < y.length; i++) {
		...
		angular.forEach($scope.resdata, function(value, key) {
			if(y[i].innerHTML===value.pageName){
			 console.log(key + ':gggggggggggggggggg ' + value.pageName);
			$scope.newresdata[j]=value;
			j=j+1;
			console.log(JSON.stringify($scope.newresdata));
			}else{
